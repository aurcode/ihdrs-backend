...
erDiagram
    ...
    users ||--o{ recognition_records : "执行"
    users ||--o{ feedback_data : "提交"
    users ||--o{ feedback_data : "审核"
    users ||--o{ operation_logs : "操作"
    users ||--o{ user_log :  "记录"
    users ||--o{ datasets : "创建"
    
    models ||--o{ recognition_records : "使用"
    models ||--o| training_tasks : "生成"
    
    training_tasks ||--o{ training_logs : "包含"
    
    recognition_records ||--o{ feedback_data : "关联"
    ...
    datasets {
        bigint dataset_id PK "数据集ID"
        varchar dataset_name "数据集名称"
        enum dataset_type "数据集类型"
        text description "描述"
        varchar file_path "文件路径"
        bigint file_size "文件大小"
        int num_classes "类别数量"
        int num_samples "样本总数"
        int train_samples "训练集样本数"
        int test_samples "测试集样本数"
        json class_names "类别名称列表"
        enum status "状态"
        bigint creator_id FK "创建者ID"
    }

    models {
        bigint model_id PK "模型ID"
        varchar model_name "模型名称"
        varchar model_version "模型版本"
        varchar model_path "模型文件路径"
    ...
    recognition_records {
        ...
        varchar image_path "图像路径"
        enum input_type "输入类型"
    ...
    training_tasks {
        ...
        bigint creator_id FK "创建者ID"
        bigint model_id FK "生成的模型ID"
        json dataset_config "数据集配置"
        json training_config "训练配置"
        enum status "任务状态"
        decimal progress "进度"
        int current_epoch "当前轮数"
        int total_epochs "总轮数"
        decimal best_accuracy "最佳准确率"
    }
    ...
    training_logs {
        ...
        decimal loss "损失值"
        decimal accuracy "准确率"
        decimal val_loss "验证损失"
        decimal val_accuracy "验证准确率"
        datetime timestamp "时间戳"
    }

    feedback_data {
        bigint feedback_id PK "反馈ID"
        bigint record_id FK "识别记录ID"
        bigint user_id FK "反馈用户ID"
        int original_result "原始结果"
        int correct_result "正确结果"
        enum feedback_type "反馈类型"
        enum status "反馈状态"
        bigint reviewer_id FK "审核者ID"
    }
