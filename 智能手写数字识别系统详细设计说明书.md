...
classDiagram
    ...
    Feedback "1" --> "1" RecognitionRecord : belongs to
...
Python模型服务是基于Flask框架构建的机器学习推理与训练服务，负责手写数字识别模型的加载、预测、训练及管理。该服务采用模块化设计，支持多种深度学习模型架构，并提供RESTful API与Spring Boot主服务进行通信。
...
graph TB
    ...
    subgraph "API层"
        MB[model_bp<br/>模型管理API]
        RB[recognition_bp<br/>识别API]
        TB[training_bp<br/>训练API]
    end
    
    subgraph "服务层"
        MS[ModelService<br/>模型服务]
        TS[TrainingService<br/>训练服务]
    ...
    subgraph "工具层"
        VAL[Validators<br/>数据验证]
        LOG[Logger<br/>日志工具]
        TR[TasksRegistry<br/>任务注册]
    end
...
graph LR
    ...
    subgraph "服务模块"
        S1[model_service.py]
        S2[training_service.py]
        S3[image_processor.py]
        S4[tasks_registry.py]
    end
...
│   └── training.py               # 训练接口
├── services/                     # 业务服务层
│   ├── model_service.py          # 模型加载与预测服务
│   ├── training_service.py       # 模型训练服务
│   ├── image_processor.py        # 图像预处理服务
│   └── tasks_registry.py         # 训练任务注册表
├── models_code/                  # 模型定义层
...
```mermaid
graph LR
    subgraph "模型管理 /models"
        M1[POST /models/activate<br/>激活模型]
    end
    
    subgraph "识别服务 /recognize"
        R1[POST /recognize<br/>单数字识别]
        R2[POST /recognize_multi<br/>多数字识别]
    end
    
    subgraph "训练服务 /train"
        T1[POST /train<br/>启动训练]
        T2[POST /train/cancel<br/>取消训练]
    end
```
...
| model_id | integer | 否   | 使用的模型ID，默认为1 |
...
sequenceDiagram
    ...
    participant A as recognition_bp
    participant IP as ImageProcessor
    participant MS as ModelService
    
    C->>A: POST /recognize_multi
    A->>A: Base64解码
    A->>IP: segment_digits(image_bytes)
    IP->>IP: 图像预处理
    IP->>IP: 轮廓检测与分割
    IP-->>A: 数字图像列表
    
    loop 每个数字
        A->>MS: predict(digit_img)
        MS-->>A: 预测结果
    end
...
| trainingConfig | string | 是   | 训练配置JSON字符串   |
| datasetConfig  | string | 是   | 数据集配置JSON字符串 |
...
| learningrate          | float  | 0.001  | 学习率           |
| optimizer             | string | adam   | 优化器类型       |
| modeltype             | string | cnn    | 模型类型         |
| useAugmentation       | bool   | false  | 是否使用数据增强 |
| augmentationStrength  | string | medium | 数据增强强度     |
| earlyStoppingPatience | int    | 0      | 早停耐心值       |
| lrScheduler           | string | none   | 学习率调度器     |

## 4.5 服务层详细设计

### 4.5.1 ModelService 模型服务
...
classDiagram
    class ModelService {
        ...
        -int active_model_id
        -dict model_metadata
        -dict model_input_type
        +load_default_model() bool
        +predict(image, model_id) dict
        +save_model(model, name, metadata) int
        +list_models() dict
        +get_active_model_id() int
        -_warmup_model(model, input_type) void
    }
```
...
| 方法               | 功能描述                                      |
| ------------------ | --------------------------------------------- |
| load_default_model | 启动时加载默认模型，检测输入类型并预热        |
| predict            | 执行预测，自动适配模型输入格式（flatten/cnn） |
| save_model         | 保存训练好的模型到文件系统并注册到内存        |
| _warmup_model      | 使用dummy数据预热模型，减少首次预测延迟       |

**模型输入类型自动检测：**

```mermaid
flowchart TD
    A[加载模型] --> B{检查input_shape}
    B -->|shape=None, 784| C[flatten类型<br/>全连接网络]
    B -->|shape=None, 28, 28, 1| D[cnn类型<br/>卷积神经网络]
    B -->|其他| E[unknown类型]
    
    C --> F[预测时reshape为1, 784]
    D --> G[预测时reshape为1, 28, 28, 1]
    E --> H[尝试自动适配]
```

### 4.5.2 ImageProcessor 图像处理器

```mermaid
classDiagram
    class ImageProcessor {
        -tuple target_size
        +preprocess_for_recognition(bytes) ndarray
        +segment_digits(bytes) List~ndarray~
        +validate_image(bytes, max_size) bool
        -_preprocess_pipeline(ndarray) ndarray
        -_extract_roi(ndarray) ndarray
        -_resize_with_pad(ndarray) ndarray
        -_center_image(ndarray) ndarray
    }
```
...
flowchart TD
    subgraph "单数字预处理"
        A1[原始图像字节] --> B1[PIL图像加载]
        B1 --> C1[RGB转换/透明背景处理]
        C1 --> D1[转numpy数组]
        D1 --> E1[转灰度图]
        E1 --> F1[去噪 fastNlMeansDenoising]
        F1 --> G1[OTSU二值化]
        G1 --> H1[提取ROI]
        H1 --> I1[resize到28x28]
        I1 --> J1[归一化到0-1]
...
classDiagram
    class TrainingService {
        ...
        -str springboot_url
        -bool is_cancelled
        +start_training(config, dataset) void
        +report_progress(data) void
        +report_batch_progress(data) void
        +report_completion(result) void
        +report_failure(error) void
        -create_optimizer(name, lr) Optimizer
        -create_lr_scheduler(type, lr, epochs) Callback
        -create_data_augmentation(strength) Sequential
    }
    ...
    class ProgressCallback {
        ...
        -int global_step
        +on_epoch_begin(epoch, logs) void
        +on_train_batch_end(batch, logs) void
        +on_epoch_end(epoch, logs) void
    }
    
    TrainingService --> ProgressCallback
```
...
sequenceDiagram
    ...
    TS->>TS: 解析配置
    TS->>TS: 加载数据集
    TS->>MF: create_model(type, config)
    MF-->>TS: 返回模型
    TS->>TF: model.compile()
    TS->>TF: model.fit()
    
    loop 每个Epoch
        TF->>TS: on_epoch_begin
        TS->>SB: 报告epoch开始
        
        loop 每5个Batch
            TF->>TS: on_train_batch_end
            TS->>SB: 报告batch进度
        end
...
| 优化器  | 类名    | 特点                   |
| ------- | ------- | ---------------------- |
| adam    | Adam    | 自适应学习率，默认推荐 |
| adamw   | AdamW   | Adam + 权重衰减        |
| sgd     | SGD     | 带动量的随机梯度下降   |
| rmsprop | RMSprop | 适合RNN                |
...
| ------ | ------------------------------------------------ |
| light  | 水平翻转 + 轻微旋转(5°)                          |
| medium | 水平翻转 + 旋转(10°) + 缩放(10%)                 |
| strong | 双向翻转 + 旋转(20°) + 缩放(20%) + 平移 + 对比度 |
...
graph TB
    ...
    subgraph "高级模型"
        ACNN[Advanced CNN<br/>6层卷积 + BatchNorm]
        RES[ResNet<br/>残差连接]
        VGG[VGG<br/>深层卷积]
        MOB[MobileNet<br/>深度可分离卷积]
    end
...
| 模型类型     | 层数            | 特点               | 适用场景     |
| ------------ | --------------- | ------------------ | ------------ |
| CNN          | 3卷积+1全连接   | 轻量快速           | 简单数字识别 |
| Advanced CNN | 6卷积+1全连接   | BatchNorm防过拟合  | 复杂图像     |
| ResNet       | 6残差块         | 残差连接防梯度消失 | 深度网络     |
| VGG          | 8卷积+2全连接   | 经典架构           | 通用图像分类 |
| MobileNet    | 6深度可分离卷积 | 参数少、速度快     | 移动端部署   |
...
flowchart TD
    subgraph "基础CNN结构"
        I[Input 28x28x1] --> C1[Conv2D 32 3x3]
        C1 --> BN1[BatchNorm]
        BN1 --> P1[MaxPool 2x2]
        P1 --> C2[Conv2D 64 3x3]
        C2 --> BN2[BatchNorm]
...
flowchart TD
    A[接收请求数据] --> B{是否为dict?}
    B -->|否| C[返回: 请求数据必须是JSON对象]
    B -->|是| D{包含image字段?}
    D -->|否| E[返回: 缺少必需的image字段]
    D -->|是| F{image是字符串?}
    F -->|否| G[返回: image字段必须是字符串]
    F -->|是| H{image长度>0?}
    H -->|否| I[返回: image数据不能为空]
    H -->|是| J{Base64格式有效?}
    J -->|否| K[返回: 无效的Base64数据格式]
    J -->|是| L{model_id类型正确?}
    L -->|否| M[返回: model_id必须是整数]
    L -->|是| N[返回: None 验证通过]
```
...
tasks = {
    "task_id_1": {
        "thread": <Thread对象>,      # 训练线程
        "service": <TrainingService>  # 训练服务实例
    },
    "task_id_2": { ...}
}
```
...
sequenceDiagram
    ...
    participant MS as ModelService
    participant Model as TensorFlow模型
    
    App->>MS: load_default_model()
    MS->>Model: keras.models.load_model()
    Model-->>MS: 模型对象
    
    loop 预热3次
        MS->>Model: predict(dummy_input)
        Model-->>MS: 预测结果
    end
...
**优化策略：**
- 使用 `tf.data.AUTOTUNE` 自动调整预取缓冲区大小
- 并行数据预处理 `num_parallel_calls=tf.data.AUTOTUNE`
- 数据增强在GPU上执行
...
| 模型缓存     | 使用字典缓存已加载模型 |
| 惰性加载     | 按需加载非活跃模型     |
| 图像流式处理 | 使用BytesIO避免文件IO  |

## 4.10 与Spring Boot服务通信

### 4.10.1 回调接口

| 接口路径                                    | 方法 | 功能              |
| ------------------------------------------- | ---- | ----------------- |
| /api/training/tasks/{taskId}/progress       | POST | 报告epoch级别进度 |
| /api/training/tasks/{taskId}/batch-progress | POST | 报告batch级别进度 |
| /api/training/tasks/{taskId}/complete       | POST | 报告训练完成      |
| /api/training/tasks/{taskId}/fail           | POST | 报告训练失败      |
...
```json
{
    "finalAccuracy": 0.9923,
    "finalLoss": 0.0234,
    "modelPath": "models/model_task123_1701500000.h5",
    "trainingSamples": 60000,
    "testSamples": 10000,
    "modelSize": 1234567,
    "confusionMatrix": [[980, 0, 1, ...], ...],
    "classNames": ["0", "1", "2", ...]
}
```
