...
| T4.15  | 管理端仪表板页面          | 刘家乐 | DashboardView.vue      |
| T4.16  | 仪表板统计卡片组件        | 刘家乐 | StatCard.vue           |
| T4.17  | 仪表板图表组件            | 刘家乐 | DashboardCharts.vue    |
| T4.18  | 管理端用户管理页面        | 刘家乐 | UserManagement.vue     |
| T4.19  | 用户列表表格组件          | 郑青惟 | UserTable.vue          |
| T4.20  | 用户编辑对话框            | 郑青惟 | UserEditDialog.vue     |
...
| 任务ID | 任务名称                | 负责人 | 产出物                 |
| ------ | ----------------------- | ------ | ---------------------- |
| T6.1   | 管理端识别历史页面      | 刘家乐 | HistoryManagement.vue  |
| T6.2   | 识别记录表格组件        | 刘家乐 | RecordTable.vue        |
| T6.3   | 高级搜索组件            | 刘家乐 | AdvancedSearch.vue     |
| T6.4   | 管理端反馈管理页面      | 刘家乐 | FeedbackManagement.vue |
| T6.5   | 反馈审核对话框          | 刘家乐 | ReviewDialog.vue       |
| T6.6   | 管理端统计分析页面      | 刘家乐 | StatisticsView.vue     |
| T6.7   | 趋势图表组件            | 刘家乐 | TrendCharts.vue        |
| T6.8   | 分布图表组件            | 刘家乐 | DistributionCharts.vue |
| T6.9   | 管理端手写识别页面      | 安娜   | RecognitionView.vue    |
| T6.10  | 管理端画布组件          | 安娜   | DrawingBoard.vue       |
| T6.11  | 识别结果展示组件        | 安娜   | ResultDisplay.vue      |
| T6.12  | 用户端历史记录页面      | 刘家乐 | HistoryScreen.js       |
...
| T7.2   | 管理端组件测试    | 刘家乐 | 测试报告     |
| T7.3   | 用户端功能测试    | 刘家乐 | 测试报告     |
| T7.4   | 集成测试-认证流程 | 刘家乐 | 测试报告     |
| T7.5   | 集成测试-识别流程 | 刘家乐 | 测试报告     |
| T7.6   | 集成测试-训练流程 | 刘家乐 | 测试报告     |
...
| T7.14  | 模型微调-数据增强 | 刘家乐 | 优化模型     |
| T7.15  | 模型微调-参数优化 | 刘家乐 | 优化模型     |
| T7.16  | 准确率验证        | 刘家乐 | 验证报告     |
| T7.17  | 回归测试          | 全体   | 回归报告     |

#### **第8周：部署与验收（2025-12-01 ~ 2025-12-07）**

| 任务ID | 任务名称               | 负责人 | 工时(h) | 产出物             | 依赖  |
| ------ | ---------------------- | ------ | ------- | ------------------ | ----- |
| T8.1   | Spring Boot Dockerfile | 刘家乐 | 4       | Dockerfile         | M7    |
| T8.2   | Flask Dockerfile       | 刘家乐 | 4       | Dockerfile         | M7    |
| T8.3   | Docker Compose配置     | 刘家乐 | 4       | docker-compose.yml | T8.1  |
| T8.4   | Nginx配置              | 刘家乐 | 4       | nginx.conf         | T8.3  |
| T8.5   | 生产环境部署           | 刘家乐 | 8       | 部署服务           | T8.4  |
| T8.6   | 生产环境验证           | 刘家乐 | 4       | 验证报告           | T8.5  |
| T8.7   | 用户验收测试-管理端    | 刘家乐 | 8       | UAT报告            | T8.6  |
| T8.8   | 用户验收测试-用户端    | 刘家乐 | 8       | UAT报告            | T8.6  |
| T8.9   | 问题修复               | 全体   | 8       | 修复记录           | T8.7  |
| T8.10  | 用户操作手册编写       | 刘家乐 | 8       | 用户操作手册       | T8.6  |
| T8.11  | 部署运维手册编写       | 刘家乐 | 8       | 部署运维手册       | T8.5  |
| T8.12  | API接口文档完善        | 刘家乐 | 6       | API文档            | T8.6  |
| T8.13  | 详细设计说明书完善     | 刘家乐 | 8       | 详细设计说明书     | T8.6  |
| T8.14  | 项目总结报告编写       | 刘家乐 | 6       | 项目总结报告       | T8.9  |
| T8.15  | 项目验收汇报准备       | 刘家乐 | 4       | PPT演示文稿        | T8.14 |
| T8.16  | 项目验收会议           | 全体   | 4       | 验收报告           | T8.15 |
...
| 风险ID | 应对策略                                                     | 责任人 | 触发条件             |
| ------ | ------------------------------------------------------------ | ------ | -------------------- |
| R01    | 1.制定详细的HTTP接口规范<br>2.统一使用JSON数据格式<br>3.建立服务健康检查机制<br>4. 实现请求重试和超时处理 | 刘家乐 | 服务调用失败率>5%    |
| R02    | 1. 数据增强（旋转、缩放、噪声）<br>2. 尝试多种模型架构（ResNet、VGG）<br>3. 超参数调优（学习率、批次大小）<br>4.使用预训练模型微调 | 刘家乐 | 准确率<85%时触发     |
| R03    | 1. 优先使用成熟的RN组件库<br>2. 简化UI交互设计<br>3.备选方案：开发Web移动端<br>4.增加高翔的技术培训 | 刘家乐 | 第4周功能完成度<50%  |
| R04    | 1. 先使用轮询方案实现<br>2.WebSocket作为增强功能<br>3.使用现成的WebSocket库 | 刘家乐 | WebSocket调试超过2天 |
| R05    | 1. 使用较小的数据集进行开发测试<br>2.优化训练代码提高效率<br>3.考虑使用云GPU服务<br>4.预训练模型本地微调 | 刘家乐 | 训练时间>4小时/轮    |
| R06    | 1. 提前定义接口Mock数据<br>2.并行开发前后端<br>3.每周进行一次联调 | 刘家乐 | 第5周联调问题>10个   |
| R07    | 1. 单元测试覆盖率要求≥80%<br>2.集成测试覆盖核心流程<br>3.代码评审制度 | 刘家乐 | 发现严重Bug>3个      |
| R08    | 1. 分块上传大文件<br>2.异步处理数据集解析<br>3.使用Redis缓存热点数据 | 岑晓茵 | 上传100MB文件超时    |
| R09    | 1. 每周技术分享会<br>2. 代码规范培训<br>3.结对编程           | 刘家乐 | 代码评审问题>20个/周 |
| R10    | 1. 锁定依赖版本<br>2.使用Docker确保环境一致<br>3.定期更新依赖 | 刘家乐 | 依赖冲突导致构建失败 |
...
| ------------ | -------- | ----------------------- | ------ |
| 项目进度     | 每日     | 站会、任务看板          | 刘家乐 |
| 代码质量     | 每周     | 代码评审、SonarQube扫描 | 刘家乐 |
| 模型准确率   | 每周     | 测试集评估              | 刘家乐 |
| 系统性能     | 每周     | 性能测试报告            | 刘家乐 |
| 接口联调问题 | 每周     | 联调问题清单            | 刘家乐 |
| 风险状态     | 每周     | 风险评审会议            | 刘家乐 |
...
| 用例ID      | 功能模块 | 测试场景           | 前置条件     | 测试步骤                                         | 预期结果                |
| ----------- | -------- | ------------------ | ------------ | ------------------------------------------------ | ----------------------- |
| TC-AUTH-001 | 用户登录 | 正确用户名密码登录 | 用户已注册   | 1.输入正确用户名<br>2.输入正确密码<br>3.点击登录 | 登录成功，返回Token     |
| TC-AUTH-002 | 用户登录 | 错误密码登录       | 用户已注册   | 1.输入正确用户名<br>2.输入错误密码<br>3.点击登录 | 登录失败，提示密码错误  |
| TC-REC-001  | 数字识别 | 画布手写识别       | 用户已登录   | 1.在画布上书写数字5<br>2. 点击识别按钮           | 识别结果为5，置信度≥80% |
| TC-REC-002  | 数字识别 | 空白画布识别       | 用户已登录   | 1.不在画布上书写<br>2.点击识别按钮               | 提示请先书写数字        |
| TC-TRN-001  | 模型训练 | 创建训练任务       | 管理员已登录 | 1. 选择数据集<br>2.配置训练参数<br>3. 提交       | 任务创建成功，开始训练  |
| TC-TRN-002  | 模型训练 | 取消训练任务       | 训练进行中   | 1.选择运行中任务<br>2.点击取消                   | 任务状态变为已取消      |

**性能测试用例示例：**

| 用例ID | 测试场景         | 并发用户 | 持续时间 | 性能指标             | 预期结果            |
| ------ | ---------------- | -------- | -------- | -------------------- | ------------------- |
| PT-001 | 识别接口响应时间 | 1        | -        | 平均响应时间         | ≤500ms              |
| PT-002 | 识别接口并发测试 | 100      | 5min     | 平均响应时间、错误率 | ≤2s，错误率<1%      |
| PT-003 | 登录接口压力测试 | 200      | 10min    | TPS、错误率          | TPS≥50，错误率<0.5% |
| PT-004 | 文件上传性能     | 10       | -        | 100MB文件上传时间    | ≤60s                |
...
1. IEEE Std 1058-1998 软件项目管理计划规范
2. GB/T 8567-2006 计算机软件文档编制规范
3. PMBOK第六版 项目管理知识体系指南
4. Spring Boot 3.x 官方文档
5. Vue.js 3.x 官方文档
6. React Native 官方文档
7. TensorFlow 2.x 官方文档
8. Flask 官方文档
9. MySQL 8.0 参考手册
10. Docker 官方文档
